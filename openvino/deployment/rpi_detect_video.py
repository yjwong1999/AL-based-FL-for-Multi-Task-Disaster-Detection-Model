...
def parse_args() -> argparse.Namespace:
    ...
    args.add_argument('-i', '--input', required=True, type=str, help='Required. Path to an video file.')
    args.add_argument('-o', '--output', type=str, default=None,
                      help='Optional. Path to output video file')
...
def main():  # noqa
    ...
    # ---------------------------Step 6. Prepare video loading---------------------------------------------------------------------
    try:
        vid = cv2.VideoCapture(int(args.input))
    except:
        vid = cv2.VideoCapture(args.input)
    assert vid.isOpened(), "Can't open " + str(args.input)
    ...
    while True:
        ...
        # ---------------------------Step 7.1. Do inference---------------------------------------------------------------------- UNTIL HERE
        #log.info('Starting inference in synchronous mode')
        res = exec_net.infer(inputs={input_blob: image})

        # ---------------------------Step 7.2. Custom Process output--------------------------------------------------------------------
